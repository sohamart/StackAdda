...
const SavedVideos = () => {
  ...
  const fetchSavedShorts = async () => {
    try {
      const res = await API.get(`/shorts/user/saved`);
      if (res.data.success) {
  ...
  if (shorts.length === 0 && !loading) {
    return (
      <div className="flex h-screen flex-col items-center justify-center bg-black text-white pt-20 px-4 text-center">
        <div className="h-24 w-24 rounded-full bg-white/5 flex items-center justify-center mb-6 border border-white/10 shadow-[0_0_30px_rgba(249,115,22,0.15)]">
          <BookmarkX size={48} className="text-orange-500/80" />
        </div>
        <h2 className="text-3xl font-bold text-white mb-3">No Saved Videos</h2>
        <p className="text-white/50 max-w-md mx-auto mb-8 text-lg">
          You haven't saved any shorts yet. Discover new content and save your favorites to watch them later.
        </p>
        <Link 
          to="/shorts"
          className="bg-orange-600 hover:bg-orange-500 text-white font-semibold py-3 px-8 rounded-full transition-all hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(249,115,22,0.3)]"
        >
          Explore Shorts
        </Link>
      </div>
  ...
  return (
    <>
    <SEO title="My Saved Videos" description="Watch your saved bite-sized coding tutorials and tips on Stack Adda." canonicalUrl="/student/saved-videos" />
    <div className="fixed inset-0 z-40 bg-black pt-20 md:pt-24 lg:pt-28">
      <h1 className="sr-only">My Saved Videos</h1>
      ...
      <div className="relative z-10 h-full w-full">
        ...
        <div className="h-20 w-full flex flex-col items-center justify-center snap-start pb-8">
          ...
          <p className="text-white/50 text-sm font-medium">End of saved shorts.</p>
        </div>
      </div>
      </div>
    </div>
